"""Version-checked extraction of NTA work identity from FlashInfer plans."""
...
def _read_i32(workspace: torch.Tensor, byte_offset: int, count: int) -> list[int]:
...
def _extract(
    ...
    if bool(plan[graph_index]) and bool(plan[split_index]):
        ...
        active = [bool(value) for value in workspace[mask_offset : mask_offset + padded].cpu()]
...
def paged_prefill_schedule(wrapper: object) -> Schedule:
    # In 0.6.12, disable_split_kv narrows INT64_MAX * page_size to an
    # IdType zero. The scheduler still emits exactly one KV tile per Q tile.
