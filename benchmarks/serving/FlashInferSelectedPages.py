...
def phase_program() -> JitPhaseProgram:
...
def main() -> int:
    ...
    for trial in range(arguments.trials):
        ...
        nta_retained_samples.append(
            event_sample(nta_retained_pipeline_call, arguments.iterations)
        )
    ...
    selected_mode_us = (
        nta_cold_median if demand_plan.mode == "indexed" else overfetch_median
    )
    report: dict[str, Any] = {
        "schema": 1,
        ...
        "online_policy_speedup_over_forced_overfetch": (
            overfetch_median / selected_mode_us
        ),
