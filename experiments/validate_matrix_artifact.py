...
def _validate_activation(record: dict[str, Any]) -> None:
    ...
    _require(record["ablation_applied"] is applied, f"incorrect ablation scope for {arm}/{ablation}")
    mode = record["execution_mode"]
    counters = record["activation_counters"]
    work_units = int(record["work_units"])
    _require(counters["exact_demand_bindings"] == work_units, f"exact demand did not execute for {arm}/{ablation}")
    if not applied:
        return
    if ablation == "host_demand":
        ...
        _require(counters["host_demand_materializations"] > 0, "host-demand ablation never materialized demand")
    elif ablation == "batch_readiness":
        _require(mode["readiness"] == "batch", "batch-readiness ablation did not use a batch boundary")
        _require(counters["batch_readiness_barriers"] > 0, "batch-readiness ablation never crossed a barrier")
    elif ablation == "coarse_granularity":
        _require(mode["granularity"] == "coarse", "coarse-granularity ablation was not active")
        _require(record["group_count"] == 1, "coarse-granularity ablation did not collapse groups")
    elif ablation == "manual_mapping":
        _require(mode["mapping"] == "manual", "manual-mapping ablation was not active")
        _require(counters["manual_mapping_sites"] > 0, "manual-mapping ablation never mapped a site")
    elif ablation == "shadow_generation_checks":
        _require(mode["generation_checks"] == "shadow", "generation checks stayed on the hot path")
        _require(counters["request_generation_checks_shadow"] > 0, "shadow generation checks never executed")
        _require(counters["request_generation_checks_hot"] == 0, "hot generation checks were not disabled")
    elif ablation == "admission_feedback":
        _require(mode["admission_feedback"] is False, "admission feedback was not disabled")
        _require(counters["admission_feedback_decisions"] == 0, "admission feedback still executed")
    elif ablation == "unbounded_staging":
        _require(mode["bounded_staging"] is False, "bounded staging was not disabled")
        _require(record["staging_high_water_units"] == record["selected_units"], "full promotion was not recorded")
...
def validate(artifact: dict[str, Any], *, require_all_ablations: bool) -> None:
    ...
    _require(measurement.get("serving_evidence") is False, "synthetic artifact cannot claim serving evidence")
    _require(measurement.get("timing_is_modeled") is True, "synthetic timing must be labeled modeled")
    ...
    _require(set(ablations) <= set(_ABLATION_TARGETS), "artifact contains an unknown ablation")
    if require_all_ablations:
        _require(set(ablations) == set(_ABLATION_TARGETS), "artifact did not execute all ablations")
    ...
    for record in records:
        ...
        _require(record["measurement"]["serving_evidence"] is False, "record claims serving evidence")
        _require(
            isinstance(record["stratum"], dict)
            and _REQUIRED_STRATUM_FIELDS <= set(record["stratum"]),
            "record has incomplete evaluation strata",
        )
        _require(record["tier"] in {"hbm", "host_mem", "nvme", "dax"}, "record uses an undeclared tier")
        _require(float(record["littles_law_residual"]) <= 1e-9, "Little's-law residual is non-zero")
        _require(record["selected_units"] <= record["candidate_units"], "selected demand exceeds candidate demand")
        _require(record["useful_bytes"] <= record["physical_bytes"] or record["arm"] == "B0", "physical bytes are less than useful bytes")
    ...
    for key, group in grouped.items():
        _require(len(group) == expected_per_group, f"incomplete arm/ablation coverage for {key}")
        trace_hashes = {record["demand_trace_hash"] for record in group}
        _require(len(trace_hashes) == 1, f"demand trace changed across matched arms for {key}")
        demand_shapes = {
            (record["candidate_units"], record["selected_units"], record["useful_bytes"])
            for record in group
        }
        _require(len(demand_shapes) == 1, f"exact demand shape changed across matched arms for {key}")
