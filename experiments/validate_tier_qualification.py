...
def _validate_entry(entry: dict[str, Any], tier: str) -> None:
    ...
    if tier in {"hbm", "host_mem"}:
        ...
        return
    if tier == "dax":
        _require(classification == "nta-dax-qualification",
                 "DAX report is not the devdax qualification report")
        _require(report.get("qualified") is True,
                 "DAX mapping qualification is not true")
        _require(report.get("verification_failures") == 0,
                 "DAX qualification has verification failures")
        _require(report.get("direct_device_visible") is True,
    ...
    _require(gpu.get("outstanding") == 0,
             "NVMe GPU-controlled run has outstanding commands")
    ...
    _require(_finite(threshold) and float(threshold) > 0,
...
def validate(document: dict[str, Any], *, required_tiers: Iterable[str] = ALL_TIERS) -> dict[str, Any]:
    ...
    _require(document.get("schema") == 1 and
             document.get("classification") == "nta-tier-qualification",
             "unsupported tier qualification document")
    _require(document.get("demand_semantics") == "exact",
...
def validate_file(path: Path, *, required_tiers: Iterable[str] = ALL_TIERS) -> dict[str, Any]:
...
def main() -> int:
    ...
    parser.add_argument(
        "--required-tier", action="append", dest="required_tiers",
        choices=ALL_TIERS, help="tier that must be qualified (repeatable; default: all)",
    )
