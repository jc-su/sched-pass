...
def main() -> None:
    ...
    assert (
        "returned to the nvme driver but no live namespace appeared" in vfio_script
    )
    ...
    control_plane = (
        ROOT / "runtime" / "host" / "NvmeVfioControlPlane.cpp"
    ).read_text(encoding="utf-8")
    ...
    bridge = (
        ROOT / "kernel" / "nta_nvme_p2p" / "nta_nvme_p2p.c"
    ).read_text(encoding="utf-8")
    for required in (
    ...
    runtime_c = (ROOT / "include" / "nta" / "RuntimeC.h").read_text(
        encoding="utf-8"
    )
    assert "nta_jit_phase_progress_nvme_until_idle" in runtime_c
    epoch = (ROOT / "python" / "nta_runtime" / "epoch.py").read_text(
        encoding="utf-8"
    )
